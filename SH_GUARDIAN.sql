...
CREATE TABLE IF NOT EXISTS `cartao` (
  ...
  `CARTAO_NUM` varchar(12) DEFAULT NULL,
  `TIPO` enum('CREDITO','DEBITO') DEFAULT NULL,
  `ID_CLIENTE` int(11) DEFAULT NULL,
  PRIMARY KEY (`ID_CARTAO`),
  KEY `FK_CARTAO` (`ID_CLIENTE`),
  CONSTRAINT `FK_CARTAO` FOREIGN KEY (`ID_CLIENTE`) REFERENCES `clientes` (`ID`) ON DELETE CASCADE ON UPDATE CASCADE
...
CREATE TABLE IF NOT EXISTS `telefone` (
  ...
  CONSTRAINT `FK_FONE` FOREIGN KEY (`ID_CLIENTE`) REFERENCES `clientes` (`ID`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_uca1400_ai_ci;
